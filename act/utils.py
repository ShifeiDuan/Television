...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(self, episode_ids, dataset_dir, camera_names, norm_stats, episode_len, chunk_size, history_stack=0):
        ...
        for i, episode_id in enumerate(self.episode_ids):
            ...
            #change it to either ee or qpos (joint_pos + ee_pose) depending on the model input
            # joint_pos = np.array(root['observation/state/joint_pos'])
            ee_pose   = np.array(root['observation/state/ee_pose'])
            gripper = np.array(root['observation/state/joint_pos'])[:, [12, 14]]  # (T, 2)
            # self.states.append(np.concatenate([joint_pos, ee_pose], axis=-1))
            self.states.append(np.concatenate([ee_pose, gripper], axis=-1))
            for cam_name in self.camera_names:
...
def get_norm_stats(dataset_dir, episode_files):
    ...
    for episode_file in episode_files:
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            ee_pose = root['observation/state/ee_pose'][()]                # (T, 18)
            gripper = root['observation/state/joint_pos'][()][:, [12, 14]] # (T, 2)
            qpos = np.concatenate([ee_pose, gripper], axis=-1)
...
def load_data(dataset_dir, camera_names, batch_size_train, batch_size_val, chunk_size):
    ...
    train_dataloader = DataLoader(train_dataset, batch_sampler=batch_sampler_train, pin_memory=True, num_workers=10, prefetch_factor=2)
    val_dataloader = DataLoader(val_dataset, batch_sampler=batch_sampler_val, pin_memory=True, num_workers=10, prefetch_factor=2)
